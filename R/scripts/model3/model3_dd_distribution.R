...
max_dd <- max(dd_per_fam$fam_dd) # 11.31
...
ggplot(dd_per_fam, aes(x = fam_dd)) + 
  ...
  labs(title = "Average DD per family (Q branches included)",
       x = "DD per family",
       y = "Nr. of families") +
  theme_clean() + th +
  scale_x_continuous(breaks = c(0,4,8,12), limits = c(-1, 14))

ggsave("./results/model3/methods_DD_dist_e_m1.jpg", 
...
ggsave("./results/model3/methods_fitted_DD_dist_e.jpg", 
       width = 1600, height = 1200, units = "px")

#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
# NEW PART: HERE I WANT TO CALCULATE DD PER BRANCH

max_dd <- max(dd_m1$dd) # 11.31
# Normalize the dd per family so that it is a value between 0 and 1
dd_m1$dd_norm <- dd_m1$dd / max_dd

ggplot(dd_m1, aes(x = dd)) + 
  geom_histogram(binwidth = 1) +
  labs(title = "Average DD per branch",
       x = "DD",
       y = "Nr. of branches") +
  theme_clean() + th +
  scale_x_continuous(breaks = c(0,4,8,12), limits = c(-1, 14))

ggsave("./results/model3/methods_DD_dist_per_branch_e_m1.jpg", 
       width = 1600, height = 1200, units = "px")

dist_stats <- descdist(dd_m1$dd_norm, discrete = F)
mean_val <- dist_stats$mean
var_val <- dist_stats$sd^2
...
ggsave("./results/model3/methods_fitted_DD_dist_per_branch_e.jpg", 
       width = 1600, height = 1200, units = "px")

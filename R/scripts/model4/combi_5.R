...
mean(sec_parameters$quality)
median(sec_parameters$quality)
hist(sec_parameters$quality)
mean(ter_parameters$quality)
median(ter_parameters$quality)
hist(ter_parameters$quality)
...
first_qualities <- rbeta(nr_of_families, shape1 = 2.165, shape2 = 3.295)
...
quality_dist <- "first_qualities"
quality_dist_2 <- 'generate_fixed_q_vector(prev_parameters)'
quality_noise <- TRUE
q_noise_dist <- "round(rnorm(cells[i], mean = quality[i], sd = 0.025), digits = 2)"
uniform_fam <- FALSE
ASD <- FALSE
